    ...
    public class PeliculasController : ControllerBase
    {
        ...
        public async Task<ActionResult<PeliculaVisualizarDTO>> Get(int id)
        {
            ...
            if (await context.VotosPeliculas.AnyAsync(x => x.PeliculaId == id))
            {
                ...
                if (HttpContext.User.Identity!.IsAuthenticated)
                {
                    ...
                    var votoUsuarioDB = await context.VotosPeliculas.FirstOrDefaultAsync(x => x.PeliculaId == id
                            && x.UsuarioId == usuarioId);

                    if (votoUsuarioDB is null)
                    {
        ...
        public async Task<ActionResult<int>> Post(Pelicula pelicula)
        {
            if (!string.IsNullOrWhiteSpace(pelicula.Poster))
            {
                ...
                pelicula.Poster = await almacenadorArchivos.GuardarArchvio(poster, "jpg", contenedor);
            }
        ...
        public async Task<ActionResult> Put(Pelicula pelicula)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pelicula.Poster))
            {
                ...
                pelicula.Poster = await almacenadorArchivos.EditarArchivo(posterImagen, ".jpg", contenedor, peliculaDB.Poster!);
            }
